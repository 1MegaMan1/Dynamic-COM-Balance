...
cost.sub(1,1).losi = 2;
cost.sub(1,1).W = 3;                                         % cost function width
cost.sub(1,1).z = 18*pi/20;                                         % target state
cost.sub(1,1).expl = 0;                                       % exploration parameter
cost.sub(1,1).angle = plant.angi;                             % angle variables in cost
cost.sub(1,1).p = 1.0;                                        % length of pendulum
cost.sub(1,1).t = pi;

cost.sub(2,1).fcn = @lossHinge;
cost.sub(2,1).losi = 2;
cost.sub(2,1).a = 7*pi/8;
cost.sub(2,1).b = [pi+cost.zmp pi-cost.zmp]; 
...
opt.length = 75/2;                         % max. number of line searches
