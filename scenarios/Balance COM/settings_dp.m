...
policy.fcn = @(policy,m,s)conCat(@congp,@gSat,policy,m,s);% controller 
                                                          % representation
policy.maxU = [3 3];                                      % max. amplitude of 
                                                          % torques
...
cost.fcn = @lossAdd;                                 % cost function
cost.zmp = pi/8;
cost.gamma = 1;                                     % discount factor

cost.sub(1,1).fcn = @lossSat;
cost.sub(1,1).losi = 3;
cost.sub(1,1).W = 2;                                         % cost function width
cost.sub(1,1).z = 0;                                         % target state
cost.sub(1,1).expl = 0;                                       % exploration parameter
cost.sub(1,1).angle = plant.angi;                             % angle variables in cost
cost.sub(1,1).p = [.5 .5];                                        % length of pendulum

cost.sub(2,1).fcn = @lossHinge;
cost.sub(2,1).losi = 4;
cost.sub(2,1).a = 6*pi/4;
cost.sub(2,1).b = [-cost.zmp cost.zmp]; 
...
opt.length = 150/2;                        % max. number of line searches
opt.MFEPLS = 30/2;                         % max. number of function evaluations
                                         % per line search
opt.verbosity = 1;                       % verbosity: specifies how much 
opt.method = 'BFGS';                    % information is displayed during
