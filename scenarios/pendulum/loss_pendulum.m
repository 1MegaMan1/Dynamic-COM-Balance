...
%  zn_target = [cost.zmp_t_n(:); ...
%       gTrig(cost.zmp_t_n(:), zeros(numel(cost.target)), cost.angle)];
   
%  zp_target = [cost.zmp_t_n(:); ...
%      gTrig(cost.zmp_t_p(:), zeros(numel(cost.target)), cost.angle)];
